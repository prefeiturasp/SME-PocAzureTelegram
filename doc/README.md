...
Este middleware é responsável por monitorar *work items* do Azure DevOpsMiddleware Azure DevOps ↔ Telegram
📌 Visão Geral
...
🧠 Como Funciona
O código Python consome periodicamente a API do Azure DevOps (via Query).
Ele verifica se há novos itens (não notificados anteriormente).
Se encontrar novos, envia uma mensagem com os detalhes para o grupo no Telegram.
Salva localmente os IDs já notificados, evitando duplicidade.
📱 Telegram
✅ Etapas para configurar o bot
Criar o bot
Acesse o @BotFather no Telegram e envie o comando /newbot.
...
Permissões do bot no grupo
O bot precisa de permissão para:

Ler mensagens (em grupos públicos)
Enviar mensagens
Obter o ID do grupo
Utilize um bot como @userinfobot ou gere logs no próprio middleware para obter o chat_id.
...
Tolerância a falhas e backoff para a API do Azure

Criação de base de dados para armazenar os id´s enviados e notificar um grupo no Telegram automaticamente, sempre que houver novos registros de acordo com uma *query* configurada.
